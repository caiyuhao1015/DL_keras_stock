...
from keras.layers import Dense, Activation, Dropout, LSTM, Conv1D, MaxPooling1D
...
class Model():
	...
	def build_model(self, configs):
		...
		for layer in configs['model']['layers']:
			...
			kernel = layer['kernel'] if 'kernel' in layer else None
			input_shape = layer["input_shape"] if "input_shape" in layer else None
			units = layer["units"] if "units" in layer else None

			if layer['type'] == 'Conv1D':
				self.model.add(Conv1D(filters=kernel, kernel_size=4, padding="same", activation=activation,
			...
			if layer['type'] == 'Attention':
				self.model.add(Attention(units=units))
	...
	def train(self, x, y, epochs, batch_size, save_dir, filename):
		...
		save_fname = os.path.join(save_dir, '{f}.h5'.format(f=filename))
	...
	# def predict_days_after(self, data, window_size, day_after, debug=False):
	# 	if not debug:
	# 		print('[Model] Predicting {number}days After...'.format(number=day_after))
	# 		predicted = []
	# 		curr_frame = data[len(data)-window_size]
